...
#import "KxLogger.h"///kxmovie中一个打log的


#pragma mark - shaders（着色器）
...
#pragma mark - frame renderers（帧渲染器）

@protocol XXGLRenderer
- (BOOL) isValid;
- (NSString *) fragmentShader;
- (void) resolveUniforms: (GLuint) program;
- (void) setFrame: (XXVideoFrame *) frame;
- (BOOL) prepareRender;
@end

@interface XXGLRenderer_RGB : NSObject <XXGLRenderer>
{
...
@implementation XXGLRenderer_RGB
...
- (void) setFrame: (XXVideoFrame *) frame
{
    ...
    XXVideoFrameRGB *rgbFrame = (XXVideoFrameRGB *)frame;
...
@interface XXGLRenderer_YUV : NSObject <XXGLRenderer>
 {
...
@implementation XXGLRenderer_YUV
...
- (void) setFrame: (XXVideoFrame *) frame
{
    XXVideoFrameYUV *yuvFrame = (XXVideoFrameYUV *)frame;
...
- (id)initWithFrame:(CGRect)frame decoder:(nonnull XXDecoder *)decoder
{
    ...
    if (self)
    {
        ...
        if ([decoder setupVideoFrameFormat:XXVideoFrameFormatYUV]) {
            
            _renderer = [[XXGLRenderer_YUV alloc] init];
            LoggerVideo(1, @"OK use YUV GL renderer");
            
        } else {
            
            _renderer = [[XXGLRenderer_RGB alloc] init];
